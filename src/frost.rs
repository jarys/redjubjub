...
pub struct Secret<S: SpendAuth>(S::Scalar);
...
#[derive(Clone)]
struct Commitment<S: SpendAuth>(S::Point);
...
#[derive(Clone)]
pub struct ShareCommitment<S: SpendAuth>(Vec<Commitment<S>>);

/// The product of all signers' individual commitments, published as part of the
/// final signature.
pub struct GroupCommitment<S: SpendAuth>(S::Point);
...
pub struct PublicKeyPackage<S: SpendAuth> {
    ...
    /// signing operation.
    pub(crate) signer_pubkeys: HashMap<u8, Public<S>>,
...
pub fn keygen_with_dealer<R: RngCore + CryptoRng, S: SpendAuth>(
    ...
    let mut signer_pubkeys: HashMap<u8, Public<S>> = HashMap::with_capacity(num_signers as usize);
...
fn generate_shares<R: RngCore + CryptoRng, S: SpendAuth>(
    ...
    commitment.0.push(Commitment(S::basepoint() * secret.0));

    for c in &coefficients {
        commitment.0.push(Commitment(S::basepoint() * c));
    }
...
impl<S: SpendAuth> From<(u8, &SigningNonces<S>)> for SigningCommitments<S> {
    /// For SpendAuth signatures only, not Binding signatures, in RedJubjub/Zcash.
    fn from((index, nonces): (u8, &SigningNonces<S>)) -> Self {
        Self {
            index,
...
impl<S: SpendAuth> SignatureShare<S> {
    ...
    pub fn check_is_valid(
        ...
        if (S::basepoint() * self.signature) != (commitment + pubkey.0 * challenge * lambda_i) {
...
fn gen_rho_i<S: SpendAuth>(index: u8, signing_package: &SigningPackage<S>) -> S::Scalar {
...
pub fn sign<S: SpendAuth>(
    ...
    let mut bindings: HashMap<u8, S::Scalar> =
